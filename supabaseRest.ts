...
export class SupabaseRestRepository implements WorkerRepository {
  ...
  async listCandidateBookings(event: TimelyAppointmentEvent): Promise<ExistingBookingSnapshot[]> {
    ...
    const query = new URLSearchParams({
      select: 'id,timely_customer_id,client_mobile,client_email,appointment_at,booking_status,booking_services(service_name)',
      or: `(${filters.join(',')})`,
      limit: '50',
    });
  ...
  private async createBooking(
    ...
    await this.request('audit_logs', {
      ...
      body: JSON.stringify({
        ...
        details: { gmail_message_id: message.id, timing },
      }),
    });
    return bookingId;
  }
  ...
  private async updateBooking(
    ...
    await this.request('audit_logs', {
      method: 'POST',
      body: JSON.stringify({ booking_id: bookingId, action: 'booking_changed_from_timely_email', details: { gmail_message_id: message.id } }),
    });
  }
